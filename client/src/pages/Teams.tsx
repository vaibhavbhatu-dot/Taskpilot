    ...
    return (
      <div className="space-y-6 animate-fade-in">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/teams')} className="p-2 rounded-lg hover:bg-[#F1F5F9] transition-colors">
            <ArrowLeft className="w-5 h-5 text-[#64748B]" />
          </button>
          <div>
            <h1 className="text-[22px] font-semibold text-[#0F172A]">{detail.name}</h1>
            {detail.lead && (
        ...
        {detail.lead && (
          <div className="bg-white rounded-xl border border-[#E2E8F0] p-5">
            <h3 className="text-[13px] font-semibold text-[#64748B] uppercase tracking-wider mb-3">Team Lead</h3>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-[#DBEAFE] flex items-center justify-center">
                <span className="text-sm font-semibold text-[#2563EB]">{getInitials(detail.lead.fullName)}</span>
              </div>
              <div>
                <p className="text-[14px] font-medium text-[#0F172A]">{detail.lead.fullName}</p>
                <p className="text-[12px] text-[#64748B]">{detail.lead.designation || detail.lead.email}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <div className="bg-white rounded-xl border border-[#E2E8F0] overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#E2E8F0]">
                  <th className="text-left px-5 py-3 text-[12px] font-semibold text-[#64748B] uppercase">Member</th>
                  <th className="text-left px-5 py-3 text-[12px] font-semibold text-[#64748B] uppercase">Designation</th>
                  <th className="text-left px-5 py-3 text-[12px] font-semibold text-[#64748B] uppercase">Role</th>
                  <th className="text-left px-5 py-3 text-[12px] font-semibold text-[#64748B] uppercase">Tickets</th>
                </tr>
              </thead>
              <tbody>
                {detail.members?.map((member, i) => {
                  ...
                  return (
                    <tr key={member.id} className={`h-[52px] hover:bg-[#F1F5F9] ${i % 2 === 1 ? 'bg-[#F8FAFC]' : ''}`}>
                      <td className="px-5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-[#DBEAFE] flex items-center justify-center">
                            <span className="text-[11px] font-semibold text-[#2563EB]">{getInitials(member.fullName)}</span>
                          </div>
                          <div>
                      ...
                      <td className="px-5 text-sm text-[#0F172A]">{member.designation || '—'}</td>
                      <td className="px-5">
                        <span className={`inline-block px-2 py-0.5 rounded text-[11px] font-semibold ${badge.bg} ${badge.text}`}>
                          {member.role.replace('_', ' ')}
                        </span>
                      </td>
                      <td className="px-5 text-sm text-[#64748B]">{member._count?.assignedTickets || 0}</td>
                    </tr>
                  );
                })}
              </tbody>
            ...
            {(!detail.members || detail.members.length === 0) && (
              <div className="text-center py-8 text-sm text-[#64748B]">No members in this team yet</div>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {teams.map((team) => (
          <div key={team.id} className="bg-white rounded-xl border border-[#E2E8F0] p-5 hover:border-[#2563EB]/30 transition-colors">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 rounded-lg bg-[#EFF6FF] flex items-center justify-center">
                <UsersIcon className="w-5 h-5 text-[#2563EB]" />
              </div>
              <h3 className="text-[16px] font-semibold text-[#0F172A]">{team.name}</h3>
            </div>

            {team.lead && (
              <div className="flex items-center gap-2 mb-2">
                <div className="w-6 h-6 rounded-full bg-[#DBEAFE] flex items-center justify-center">
                  <span className="text-[10px] font-semibold text-[#2563EB]">{team.lead.fullName?.charAt(0)}</span>
                </div>
                <span className="text-[14px] text-[#0F172A]">{team.lead.fullName}</span>
              </div>
            )}

            <p className="text-[13px] text-[#64748B] mb-4">{team._count?.members || 0} members</p>

            <button
              onClick={() => navigate(`/teams/${team.id}`)}
              className="text-[14px] text-[#2563EB] hover:underline font-medium"
            >
              View Team
            </button>
          </div>
      ...
      {teams.length === 0 && (
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-12 text-center">
          <UsersIcon className="w-8 h-8 mx-auto text-[#94A3B8] mb-3" />
          <p className="text-sm text-[#64748B]">No teams yet. Create your first team.</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl w-full max-w-[480px] p-7 animate-fade-in">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-[18px] font-semibold text-[#0F172A]">Create Team</h2>
